...
func NewBatchService(conn clickhouse.Conn, cfg *config.Config) *BatchService {
	return &BatchService{
		conn:          conn,
		cfg:           cfg,
		interval:      cfg.GetAggregateInterval(),
...
func (s *BatchService) executeAggregation() {
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Minute)
	defer cancel()

	startTime := time.Now()
	log.Println("[BatchService] 开始聚合批次...")
	...
	log.Printf("[BatchService] 聚合批次完成，耗时: %v", time.Since(startTime))
}
...
func (s *BatchService) ExecuteOnce() {
	s.executeAggregation()
	s.executeAlertDetection()
}

func (s *BatchService) ExecuteAggregationOnly() {
	s.executeAggregation()
}

func (s *BatchService) ExecuteAlertDetectionOnly() {
	s.executeAlertDetection()
}
